    ...
    async function checkKey() {
      try {
        ...
        if (typeof window !== 'undefined' && window.aistudio && window.aistudio.hasSelectedApiKey) {
          ...
          console.log('Has API key:', has);
          setHasKey(has);
        } else {
          console.log('window.aistudio not found, defaulting to true for dev');
          setHasKey(true); 
        }
      } catch (e) {
        console.error('Error checking API key:', e);
        // Fallback to allow access if check fails
        setHasKey(true);
      } finally {
  ...
  const handleSelectKey = async () => {
    try {
      if (window.aistudio && window.aistudio.openSelectKey) {
        ...
        console.warn('window.aistudio.openSelectKey not available');
